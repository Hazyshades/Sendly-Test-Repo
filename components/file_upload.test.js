...
test('FileUpload delegates selection, upload, and state to useFileUpload', () => {
...
test('FileUpload guards empty uploads and re-entrant submissions via the hook', () => {
...
test('FileUpload exposes one labelled input with stable status and error roles', () => {
  ...
  assert.equal(
    ariaDescribedByMatches.length,
    1,
    'input should have exactly one aria-describedby attribute',
  );
